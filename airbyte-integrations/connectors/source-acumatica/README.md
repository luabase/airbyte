...
```
./gradlew :airbyte-integrations:connectors:source-acumatica:build
```
...
to generate the necessary credentials. Then create a file `secrets/config.json` conforming to the `source_acumatica/spec.yaml` file.
Note that any directory named `secrets` is gitignored across the entire Airbyte repo, so there is no danger of accidentally checking in sensitive information.
See `integration_tests/sample_config.json` for a sample config file.

**If you are an Airbyte core member**, copy the credentials in Lastpass under the secret name `source acumatica test creds`
...
```
./gradlew :airbyte-integrations:connectors:source-acumatica:airbyteDocker
```
...
To run unit tests:
```
./gradlew :airbyte-integrations:connectors:source-acumatica:unitTest
```
To run acceptance and custom integration tests:
```
./gradlew :airbyte-integrations:connectors:source-acumatica:integrationTest
```
